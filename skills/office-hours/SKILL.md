...
4. **List existing design docs for this project:**
   ...
   ls -t .claude/gstack/*-design-*.md 2>/dev/null
   ```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" .claude/gstack/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `.claude/gstack/`.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t .claude/gstack/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `.claude/gstack/{user}-{branch}-design-{datetime}.md`.
...
```bash
PROFILE=$(gstack-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
The design doc at `.claude/gstack/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
