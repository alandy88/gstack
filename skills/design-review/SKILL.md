...
```
Write to: `.claude/gstack/{user}-{branch}-design-audit-{datetime}.md`
...
```
.claude/gstack/designs/design-audit-{YYYYMMDD}/
...
eval "$(gstack-slug 2>/dev/null)" && mkdir -p .claude/gstack/
```
Write a one-line summary to `.claude/gstack/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
