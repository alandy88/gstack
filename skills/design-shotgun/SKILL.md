...
_PREV=$(find .claude/gstack/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
```bash
setopt +o nomatch 2>/dev/null || true
ls .claude/gstack/*office-hours* 2>/dev/null | head -5
```
...
**Persistent taste profile (v1 schema at `.claude/gstack/taste-profile.json`):**

Read the persistent taste profile if it exists:

```bash
_TASTE_PROFILE=.claude/gstack/taste-profile.json
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find .claude/gstack/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `.claude/gstack/designs/`. This is enforced. See DESIGN_SETUP above.
