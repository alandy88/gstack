...
for PLAN_DIR in ".claude/gstack" ".claude/gstack/plans" ".gstack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- `specialists` = the per-specialist stats object compiled in Step 4.6
- `findings` = array of per-finding records from Step 5
...
```bash
gstack-learnings-log '{"skill":"review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
