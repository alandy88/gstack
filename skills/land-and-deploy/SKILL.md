...
**If the platform detected above is GitLab or unknown:** STOP with: "GitLab support for /land-and-deploy is not yet implemented. Run `/gstack:ship` to create the MR, then merge manually via the GitLab web UI." Do not proceed.
...
if [ ! -f .claude/gstack/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat .claude/gstack/land-deploy-confirmed 2>/dev/null)
...
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > .claude/gstack/land-deploy-confirmed
```
Continue to Step 2.
...
**If C:** **STOP.** "Running `/gstack:setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to CLAUDE.md so I'll know exactly what to do next time. Run `/land-and-deploy` again when that's done."
...
3. If drift is detected (a PR landed ahead of us and `BRANCH_VERSION < NEXT_SLOT`): **STOP** and print exactly:
   ...
   Exit non-zero. Do NOT auto-bump from `/land-and-deploy` — rerunning `/gstack:ship` is the clean path (it already handles VERSION + package.json + CHANGELOG header + PR title atomically via Step 12 ALREADY_BUMPED detection).
...
- If reviews are stale: "Run `/gstack:review` or `/gstack:autoplan` to review the current code, then `/land-and-deploy` again."
...
- **Single-pass verification, not continuous monitoring.** `/land-and-deploy` checks once. `/gstack:canary` does the extended monitoring loop.
