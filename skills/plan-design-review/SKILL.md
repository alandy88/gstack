...
MUST be saved to `.claude/gstack/designs/`, NEVER to `.context/`,
...
`.claude/gstack/designs/` (user config directory, not project files).
...
|----------------|-------------|-----------|-------|
| [screen name]  | .claude/gstack/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
```bash
gstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
gstack-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
