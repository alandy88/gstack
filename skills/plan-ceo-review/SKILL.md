...
DESIGN=$(ls -t .claude/gstack/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t .claude/gstack/*-design-*.md 2>/dev/null | head -1)
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t .claude/gstack/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
eval "$(gstack-slug 2>/dev/null)" && mkdir -p .claude/gstack/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p .claude/gstack/plans/archive
# For each stale plan: mv .claude/gstack/plans/{old-plan}.md .claude/gstack/plans/archive/
```

Write to `.claude/gstack/plans/{date}-{feature-slug}.md` using this format:
...
**Persist the result:**
```bash
gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
rm -f .claude/gstack/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
```bash
gstack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
- **B)** Keep in `.claude/gstack/` only (local, personal reference)
- **C)** Skip
...
```bash
gstack-learnings-log '{"skill":"plan-ceo-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
